...
public class RushKnightEnemy : MonoBehaviour
{
    Rigidbody2D rb;
    float moveSpeed = 10f;
    float health = 150;
    ...
    void Start()
    {
        maxHealth = 180 + (PlayerPrefs.GetInt("Wave") * 2.2f);
    ...
    void Update()
    {
        ...
        if (animateAttackTree)
        {
            ...
            {
                swordAttack.Play();
                treeHealth.health = treeHealth.health - 15;
                counter = 0;
        ...
        if (health <= 0)
        {
            playerStats.p1.AddGold(5);
            if (PlayerPrefs.GetInt("Scroll2") == 0)
            {
                playerStats.p1.AddExp(5);
            }
            else if (PlayerPrefs.GetInt("Scroll2") == 1)
            {
                playerStats.p1.AddExp(8);
            }
            AddKey();
            AddBook();
        ...
        {
            sprite.color = new Color32(108, 60, 60, 255);
        }
